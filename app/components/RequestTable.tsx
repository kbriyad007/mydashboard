...
import { generateInvoice, generateWhatsAppInvoiceLink, getValue } from "./InvoiceUtils";
...
export default function RequestTable({ requests, loading, error }: Props) {
  ...
  const minimalColumns = [
    "Customer-Name", "Status", "User-Email", "Phone-Number",
    "Courier", "Product-Links", "Quantity", "Time", "Message",
  ];
  const allColumns = [
    ...minimalColumns.slice(0, 5), "Address", "Description",
                  ...
                  {columns.map((key) =>
                    key === "Product-Links" ? (
                      ...
                      <td key={key} className="px-3 py-1 space-y-1">
                        ...
                        <button
                          onClick={() => generateInvoice(req, setStatusMap)}
